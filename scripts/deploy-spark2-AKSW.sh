...
function deploy {
    ...
    cd spark-version && mvn deploy -DaltDeploymentRepository=maven.aksw.snapshots::default::https://maven.aksw.org/repository/snapshots -DskipTests -Dspark.version=$1 -DSPARK_PLATFORM=$2 $3 && cd ..
    cd dl && mvn deploy -DaltDeploymentRepository=maven.aksw.snapshots::default::https://maven.aksw.org/repository/snapshots -DskipTests -Dspark.version=$1 -DSPARK_PLATFORM=$2 $3 && cd ..
    cd dist && mvn deploy -DaltDeploymentRepository=maven.aksw.snapshots::default::https://maven.aksw.org/repository/snapshots -DskipTests -Dspark.version=$1 -DSPARK_PLATFORM=$2 $3 && cd ..
}

deploy 2.4.7 SPARK_2.4 '-P spark_2.x'
deploy 3.0.1 SPARK_3.0 '-P spark_3.x'
